...
export class PropertiesService {
  ...
  updatePropertyCount(propertyId: any) {
    this.afs
      .doc<Property>(`property/${propertyId}`)
      .valueChanges()
      .subscribe(details => {
        const count = details.AvailableSlots - 1;
        this.updateAvailableSlotsCount(count);
      });
  }

  updateAvailableSlotsCount(count) {
    this.afs.doc(`property/${count}`).update({
